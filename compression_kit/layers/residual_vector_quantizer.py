...
class ResidualVectorQuantizer(keras.layers.Layer):
    ...
    def __init__(self, num_levels, num_embeddings, embedding_dim, beta=0.25, **kwargs):
        ...
        self._lvl_perp = [keras.metrics.Mean(name=f"rvq_l{l+1}_perplexity") for l in range(self.M)]
        self._lvl_usage = [keras.metrics.Mean(name=f"rvq_l{l+1}_usage") for l in range(self.M)]
        self._lvl_bpi = [keras.metrics.Mean(name=f"rvq_l{l+1}_bits_per_index") for l in range(self.M)]
        # Aggregates
    ...
    def call(self, x, return_indices: bool = False):
        ...
        for l, (K, codebook) in enumerate(zip(self.Ks, self._codebooks)):
            ...
            self._lvl_perp[l].update_state(perp)
            self._lvl_usage[l].update_state(usage)
            self._lvl_bpi[l].update_state(H)
